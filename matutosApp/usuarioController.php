...
if (isset($_POST["funcao"]) || isset($_GET["funcao"])) {
	...
	case "salvarUsuario":
		...
		if ($resultado) {
			mysql_close($con);
			echo "<script>window.location='listarUsuarios.php';alert('USUARIO CADATRADO COM SUCESSO');</script>";
		} else {
			mysql_close($con);
	...
	case "deletarUsuario":
		...
		if ($resultado) {
			mysql_close($con);
			echo "<script>window.location='listarUsuarios.php';alert('USUARIO DELETADO COM SUCESO');</script>";
		} else {
			mysql_close($con);
